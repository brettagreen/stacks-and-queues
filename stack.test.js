const { Stack, BrowserStack, balancedBrackets } = require("./stack");
...
describe("balanced brackets", function() {
	...
	it("unbalanced", function() {
		expect(balancedBrackets('(a{bcd!efg))')).toBe(false);
	});

	it("not even close", function() {
		expect(balancedBrackets('}(x')).toBe(false);
	});
});
